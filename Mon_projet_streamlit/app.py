...
try:
    ...
    st.warning("Le fichier 'scaler.pkl' est introuvable. Le modèle a peut-être été entraîné sans mise à l'échelle.")
    scaler = None #important pour ne pas avoir d'erreur par la suite
...
# Fonction de prétraitement (ENCRODAGE ET MISE A L'ECHELLE)
def preprocess_input(input_data):
    ...
    if scaler is not None and 'Feature1' in input_df.columns: #s'il y a un scaler et que la feature1 existe
        try:
            input_df[['Feature1']] = scaler.transform(input_df[['Feature1']])
        except ValueError as e:
            st.error(f"Erreur de mise à l'échelle pour 'Feature1' : {e}. Vérifiez le type de données.")
            st.stop()
    return input_df
...
if st.button('Prédire'):
    try:
        ...
        st.write('Prédiction :', prediction[0]) #Affiche la valeur de la prédiction
